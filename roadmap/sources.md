...
| SEPTA GTFS | Transit schedules for routing | Custom: revocable, redistribution permitted, fees reservable | committed; acquired 2026-09-03 (EP-12: release `v202609060`, both feeds, SHA-256 pinned, agreement archived and sentence-checked) | Pin release tags; never republish the raw feed; re-read terms each refresh; derived aggregates position: matrices contain no feed contents — computed travel times are facts (documented in DATA-LICENSES); the CI sample is synthetic |
| OSM via Geofabrik | Street network; supermarket cross-check | ODbL | committed; acquired 2026-09-03 (EP-12: the dated `pennsylvania-260831.osm.pbf`, MD5 pinned and sidecar-checked, clipped to the county + 5 km at the `network` stage; the first Bucket B source) | Share-alike: see license buckets below |
