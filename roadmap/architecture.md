...
| 9 | `travel_times` | travel-time matrices | `curated/travel_times.parquet` | precomputed stub until M3; the real body arrives with EP-15 on a go verdict. The routing engine and its harness exist since EP-13 (`phillysim.routing`): the pinned JDK and R5 jar installed project-local by `phillysim toolchain install`, every JVM run in a sampled child process (`phillysim.routing.harness`) leaving a run record under `<data root>/runs/routing/`, and `phillysim route smoke` as the first route; no stage calls it yet |
