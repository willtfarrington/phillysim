...
|---|---|---|---|
| Source contracts | schema/license/geometry expectations per adapter, on offline fixtures | CI | yes for the eight tinycity sources (`tests/contracts/`, EP-3) and, since EP-5a, for the real sources on committed real-shaped samples (the three spine sources, `tests/contracts/test_spine_sources.py`; the SNAP retailer file since EP-6 and the TIGER roads file since EP-8b, five in all); the download path itself is tested on crafted local bytes and the suite disables sockets |
...
| Integration | tinycity synthetic fixture through all 11 stages | CI | yes (`tests/integration/`, plus the `run` / `status` / `verify` / `gate --fixture` CI steps, EP-4b and EP-7); the real pipeline's eight stages run offline on the committed samples through a fake transport, ending in a gated zone that is built into a site and driven in the browser (EP-5a–EP-8b) |
