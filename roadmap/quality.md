...
| Layer | Contents | Runs | Exists (recorded at EP-9, 2026-09-02; re-checked at EP-10, 2026-09-03) |
|---|---|---|---|
| Source contracts | schema/license/geometry expectations per adapter, on offline fixtures | CI | yes for the eight tinycity sources (`tests/contracts/`, EP-3) and, since EP-5a, for the real sources on committed real-shaped samples (the three spine sources, `tests/contracts/test_spine_sources.py`; the SNAP retailer file since EP-6, the TIGER roads file since EP-8b, and since EP-12 the OSM extract on a real clipped sample under ODbL and the SEPTA feed on a synthetic sample, seven in all); the download path itself (pinned digests, MD5 sidecars, the PBF never zip-inspected, nested zip guards) is tested on crafted local bytes and the suite disables sockets |
...
| Integration | tinycity synthetic fixture through all 11 stages | CI | yes (`tests/integration/`, plus the `run` / `status` / `verify` / `gate --fixture` CI steps, EP-4b and EP-7); the real pipeline's nine stages run offline on the committed samples through a fake transport, including the `network` stage's clip and unwrap (EP-12), ending in a gated zone that is built into a site and driven in the browser (EP-5a–EP-12) |
...
| Accessibility (automated) | axe-core on built site | CI | yes since EP-8a (zero violations asserted in the same module, Windows and Linux); since EP-8b also on the page built from the sample-built real zone (the basemap with roads), and the basemap's contrast ratios measured from the page's constants (3:1 or better where the spec binds) |
| Site build | the page built only from a gated public zone, verbatim copies, deterministic bytes, vendored library digests, no off-origin loads | CI | yes since EP-8a (`tests/test_sitebuild.py`; the `site build --fixture` CI step); since EP-8b the basemap is one of the verbatim copies |
| Performance smoke | stage runtimes/memory vs budgets on fixture | CI | yes since EP-13 (`tests/test_performance_smoke.py`: `phillysim run --fixture` as a child process under the routing harness's process-tree RSS sampler, wall under 60 s and peak RSS under 2 GiB asserted on both platforms, the numbers written to the test log); the real-run baselines stay in `phillysim/README.md` (EP-9, EP-10; the first peak-RSS numbers from the EP-13 smoke route); CI never runs the JVM |
