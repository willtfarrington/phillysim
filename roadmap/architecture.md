...
|---|---|---|---|---|
| 1 | `acquire` | authorized source adapters → immutable snapshots | `raw/<source>/<snapshot-id>/` | fixture: tinycity generated and admitted through the guards; real: TIGER tracts / CenPop / ACS (EP-5a), the USDA SNAP retailer file (EP-6), the TIGER county roads file (EP-8b), and the two routing sources (EP-12: Geofabrik's dated OSM extract with its MD5 sidecar, the first Bucket B source, and SEPTA's GTFS release asset with its SHA-256 pinned) through the guarded download path (`phillysim.download`), per-source pinned snapshot IDs, existing snapshots re-used |
...
| 8 | `network` | routing inputs (GTFS + street network) | `intermediate/network.json`; real pipeline also `intermediate/network/` | fixture: computed summary over the synthetic sources; real (EP-12, `phillysim.network`, between `basemap` and `metrics`): the OSM extract clipped to the county bounds + 5 km with pyosmium (way-complete, the clip's contract enforced in-stage; Bucket B by derivation) and SEPTA's two feed zips unwrapped as files, never expanded; the report holds the counts; no JVM (the routing harness is EP-13's) |
| 9 | `travel_times` | travel-time matrices | `curated/travel_times.parquet`; real pipeline also `intermediate/travel_times.json` | fixture: precomputed stub; real (EP-15, `phillysim.routing.stage`, between `network` and `metrics`): the M3 spike's two core runs (walk 4.8 km/h; walk+transit 4.8 km/h over the pinned Wednesday's 08:00–20:00 window, one departure per minute; percentiles 50 and 85; censored at 120 min) as the tracked plan `travel-times.json`, routed by R5 in a sampled child per run as a resumable night under `<data root>/runs/routing/` (the EP-14 driver), then concatenated in the dictionary's shape; Bucket B by derivation from the clipped OSM network; never read by `publish` before M5. The stage's fingerprint covers the two routing snapshots, the spine, the retailer layer, and the plan's parameters, so a refresh re-runs routing as an unattended run (about a quarter of an hour on the development machine); it needs the routing group and the toolchain installed and refuses otherwise. The engine and harness exist since EP-13 (`phillysim.routing`); the verdict reader, the hand check, and the walk concordance against the fallback engine (`route verdict`, `route handcheck`, `route concordance`) are EP-15's |
